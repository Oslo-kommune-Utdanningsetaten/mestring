...
def import_users_from_file(org_number=None, is_overwrite_enabled=False, is_dryrun_enabled=False):
    """Import users from files"""
    print("👉 import_users_from_file: BEGIN")
    
    if org_number:
        users_file = os.path.join(data_dir, 'schools', org_number, 'users.json')
        if not os.path.exists(users_file):
            raise Exception(f"Users file not found for school {org_number}. Fetch users first.")
        with open(users_file) as file:
            data = json.load(file)
            memberships = data.get('groups', {})
    else:
        users_file = os.path.join(data_dir, 'unified', 'users.json')
        if not os.path.exists(users_file):
            raise Exception("Users file not found. Fetch users first.")
        with open(users_file) as file:
...
def import_users_from_data(memberships_data, is_overwrite_enabled=False, is_dryrun_enabled=False):
    ...
    for group_feide_id, feide_group_memberships in memberships_data.items():
        ...
        for teacher in feide_group_memberships.get('teachers', []):
            user, user_created, user_updated = ensure_user_exists(
                teacher, is_overwrite_enabled=is_overwrite_enabled, is_dryrun_enabled=is_dryrun_enabled
            )
            if user_created:
        ...
        for student in feide_group_memberships.get('students', []):
            user, user_created, user_updated = ensure_user_exists(
                student, is_overwrite_enabled=is_overwrite_enabled, is_dryrun_enabled=is_dryrun_enabled
            )
            if user_created:
                created_users.add(user.feide_id)
            elif user_updated:
...
def ensure_roles_exist(is_dryrun_enabled=False):
    ...
    if not teacher_role and not is_dryrun_enabled:
        teacher_role = models.Role.objects.create(name='teacher', maintained_at=timezone.now())
    ...
    if not student_role and not is_dryrun_enabled:
        student_role = models.Role.objects.create(name='student', maintained_at=timezone.now())
...
def ensure_user_exists(user_data, is_overwrite_enabled=False, is_dryrun_enabled=False):
    ...
    if user:
        ...
        if not is_dryrun_enabled:
            ...
            print(f"  🧪 DRY RUN: would create user {user_data['email'] or user_data['feide_id']}")
...
def ensure_membership(user, group, role, is_dryrun_enabled=False):
    ...
    if is_dryrun_enabled:
        ...
        if getattr(role, "id", None):
            exists = models.UserGroup.objects.filter(user=user, group=group, role=role).exists()
            if exists:
                print(f"    📋 Membership exists: {user.email} -> {group.display_name}")
                return False
        print(f"    🧪 DRY RUN: would create membership {user.email} -> {group.display_name}")
        return True
