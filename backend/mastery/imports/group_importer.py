...
from .feide_api import ensure_subject  # now accepts is_dryrun_enabled

# Get data directory path
script_dir = os.path.dirname(os.path.abspath(__file__))
data_dir = os.path.join(script_dir, 'data')

def import_groups_from_file(org_number=None, is_overwrite_enabled=False, is_dryrun_enabled=False):
    """Import groups from JSON files into the database"""
    print("👉 import_groups_from_file: BEGIN")
    
    if org_number:
        # School-specific import
        groups_file = os.path.join(data_dir, 'schools', org_number, 'groups.json')
        if not os.path.exists(groups_file):
            raise Exception(f"Groups file not found for school {org_number}")
        ...
        if not os.path.exists(groups_file):
            raise Exception("Groups file not found. Run fetch groups first.")
...
def import_groups_from_data(groups_data, is_overwrite_enabled=False, is_dryrun_enabled=False):
    ...
    import_basis_groups(groups_data.get('basis', []), stats, is_overwrite_enabled, is_dryrun_enabled)
    import_teaching_groups(groups_data.get('teaching', []), stats, is_overwrite_enabled, is_dryrun_enabled)
    return stats

def import_basis_groups(basis_groups, stats, is_overwrite_enabled=False, is_dryrun_enabled=False):
...
def import_teaching_groups(teaching_groups, stats, is_overwrite_enabled=False, is_dryrun_enabled=False):
...
def ensure_group_exists(group_data, group_type, subject=None, is_overwrite_enabled=False, is_dryrun_enabled=False):
    """
...
def process_subject_for_group(group_data, stats, is_dryrun_enabled=False): 
    ...
    if existing_subject:
        ...
        # Ask feide_api.ensure_subject to fetch details; do not persist in dry run
        subject = ensure_subject(grep_code, is_dryrun_enabled=is_dryrun_enabled)
        if subject:
            stats['subjects_created'] += 1
            print(f"  ✨ Subject {'would be ' if is_dryrun_enabled else ''}created: {subject.display_name}")
            stats['groups_with_subjects'] += 1
            return subject
        else:
            stats['groups_without_subjects'] += 1
