 ...
 * Fetch Feide groups for a single school (by org number) and store them at imports/data/schools/<org>/groups.json. Returns simple counts.
 */
...
/**
 * Fetch Feide users for school
 * Fetch users/memberships for a single school (by org number) into imports/data/schools/<org>/users.json
 */
export const feideFetchUsersForSchool = <ThrowOnError extends boolean = false>(options: Options<FeideFetchUsersForSchoolData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<unknown, unknown, ThrowOnError>({
